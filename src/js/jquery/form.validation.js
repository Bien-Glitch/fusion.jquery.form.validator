let valid_right,
	...
	validator = fusion.validation.settings;
...
jQuery.fn.extend({
	...
	validateEmail: function () {
		...
		if ($(target_id).val().length > 0) {
			if (!input.match(validator.regExp.email))
				toggleValidation(target_id, $(target).parents('.form-group'), 'Please input a valid E-Mail Address', false, true);
			else
				toggleValidation(target_id, $(target).parents('.form-group'));
		} else
			toggleValidation(target_id, $(target).parents('.form-group'));
	...
	$(form_group).each(function () {
		...
		$('input, textarea', currentTarget).on({
			input: function () {
				...
				if (target.attr('type') !== 'date' && target.attr('type') !== 'email')
				...
				if ($(target_id).attr('type') === 'email')
					$(target_id).validateEmail(validator.regExp.email);
			},
			keyup: function () {
